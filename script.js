class Arc {
    ...
    generate(bias1, bias2) {
        this.radius = random(10, (Math.max(dims[0], dims[1])/2)+50)
        let awayFromCenterBias = 1;
        if (bias1){
            awayFromCenterBias =  Math.max(Math.sin((Math.PI / 180) * ((this.radius/(Math.max(dims[0], dims[1])/2))*90))*1.3, 0.7) //sin curve of value from center and caps
        //at a min of 0.4 so 40% of the way out
        }
        let firstElementsAreLargestBias = 1
        if (bias2) {
         firstElementsAreLargestBias = Math.sin((Math.PI / 180) * ((this.total-this.num)/this.total) * 90) * 8
        }
        this.weight = firstElementsAreLargestBias * awayFromCenterBias * random(5, 30)
        this.rads = random(0, 1/2 * 2 * Math.PI)
        this.start = random(0, 2*Math.PI - this.rads)
        this.speed = (random(0.1, 2)/50)
        this.biasspeed = Math.min((random(0.1, 2)/50)*(1/firstElementsAreLargestBias), this.speed)
        this.colour = hslToHex(random(230, 285), random(80, 100), random(0, Math.min(firstElementsAreLargestBias*60, 60)))
...
let AWAYFROMCENTERBIAS = true
let FIRSTDRAWNBIAS = true
...
for (let i = 0; i < arcs.length; i++) {
    arcs[i].generate(AWAYFROMCENTERBIAS, FIRSTDRAWNBIAS);
